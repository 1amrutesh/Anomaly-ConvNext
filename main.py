...
sys.path.insert(1, '../softpatch/src')
sys.path.insert(1,'/home/abhishek/digest/softpatch/convnext/src/')
sys.path.append('../convnext/src/')
def parse_args():
    # project
    ...
    # backbone
    parser.add_argument("--backbone_names", "-b", type=str, action='append', default=['convnext-base'])
    parser.add_argument("--layers_to_extract_from", "-le", type=str, action='append', default=['stages.1', 'stages.2'])
    # coreset sampler
    ...
    parser.add_argument("--subdatasets", "-d", action='append', type=str, required=False)
...
def get_dataloaders(args):
    ...
    dataloaders = []
    args.subdatasets = ['electrical_insulator', 'photovoltaic_module', 'metal_welding', 'wind_turbine', 'catenary_dropper', 'nut_and_bolt','witness_mark']
    #args.subdatasets = ['witness_mark']
    for subdataset in args.subdatasets:
        ...
        if noise >= 0:
            ...
            if path.exists():
                ...
                print("Number of noise index: ",len(noise_index))
